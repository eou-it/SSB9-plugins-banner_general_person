...
class PersonAddressExtendedPropertiesServiceIntegrationTests extends BaseIntegrationTestCase {
    ...
    @Test
    void testFetchAllBySurrogateIds() {
        def personAddress = newPersonAddress()
        personAddress.save(failOnError: true, flush: true)
        assertNotNull personAddress.id
        def personAddExtendedProperties = personAddressExtendedPropertiesService.fetchAllBySurrogateIds([personAddress.id])[0]
    ...
    void testCreateNotAllowed() {
        shouldFail(ApplicationException) {
            personAddressExtendedPropertiesService.create([personAddressExtendedProperties: new PersonAddressExtendedProperties()])
        }
    }
    ...
    void testDeleteNotAllowed() {
        shouldFail(ApplicationException) {
            personAddressExtendedPropertiesService.delete([personAddressExtendedProperties: new PersonAddressExtendedProperties()])
        }
    }
